...
import com.dFarm.qa.dFarm.pageobjects.Login;
import com.dFarm.qa.dFarm.pageobjects.Logout;
 ...
 * The Class BaseTest
 *
 * @implNote This Class is used to set to perform the pre and post test actions
 */
public class BaseTest {
    ...
    public void beforeSuite(ITestContext ctx) throws FlexFrameWorkRunTimeException, IOException {
        ...
        // Setting metaData info for centralized reports
    ...
    public void testSetup(Method method, ITestContext ctx){
        try {
            ...
            if (AllDataHolder.getEnvironment().equalsIgnoreCase("Prod")){
                return;
            }else {
                login();
            }
    ...
    public void endTest(ITestContext ctx, Method method){
        try {
            String logoutSkip;
            logoutSkip = AllDataHolder.getTestData().get(AllDataHolder.getCurrentTestCaseId(), "logoutSkip");
            //Logout from GUI check for valid userIdAndDriver
            boolean validUserandDriver = !AllDataHolder.getUserId().equals("") && AllDataHolder.getDriver() !=null;
            if (validUserandDriver && logoutSkip.equalsIgnoreCase("FALSE")){
                Logout.logout();
            }
        }catch (Exception e){
    ...
    /**
     * @implNote Method to reuse the existing login or use the existing login
     * @throws ParseException
     * @throws FlexFrameWorkRunTimeException
     */
    public void login() throws ParseException, FlexFrameWorkRunTimeException{
        AllDataHolder.setTestStartTime(GlobalHelpers.getCurentCstReportTime());
        String userId = this.getUser();
        AllDataHolder.getUtil().report(DFarmConstants.INFO, "Get User ID", "User ID Successful", "User ID =" + userId);
        Login.loginWithUser(userId);
    }

    /**
     * @implSpec  To fetch the user if from service and set it so that the @Test will be able to consume it
     * @return userId
     */
    public String getUser() throws FlexFrameWorkRunTimeException {
        String type, userId = "";
        //getting type from context
        type = AllDataHolder.getTestData().get(AllDataHolder.getCurrentTestCaseId(), ServiceAndSauce.TYPE);
        boolean isSauce = DriverMode.isSauce();
        AllDataHolder.setType(type);
        AllDataHolder.setUserId("");
        if (AllDataHolder.getIsServiceData() && !StringUtils.isEmpty(type)){
            AllDataHolder.getUtil().report(2, "Check if type is specified", "type should be specified",
                    "Type column/Value not specified in the local test data sheet. resuming test execution with local data");
            userId = AllDataHolder.getTestData().get(AllDataHolder.getCurrentTestCaseId(), DFarmConstants.USERNAME);
        }
        //to check if the data is coming from the service
        if (AllDataHolder.getIsServiceData() && !StringUtils.isEmpty(type)){
            try {
                //If running on sauce get userId
                if (isSauce){
                    userId = GetTestData.getUserID(type);
                }else {
                    do {
                        userId = GetTestData.getUserID(type);
                    }while (AllDataHolder.userIdMap.containsValue(userId));
                }
        ...
        if (StringUtils.isEmpty(userId)){
            AllDataHolder.setIsLastRun(true);
            AllDataHolder.getUtil().report(2, "Check if user id is empty", "User Id is Empty",
                    "user id is missing from TestData.xlsx file under Resources");
        }
        AllDataHolder.setUserId(userId);
        return userId;
    }
